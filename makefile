...
$(LOGISTIC_REGRESSION_CHECK): | kinit_check hdfs_check spark_check

clean_$(LOGISTIC_REGRESSION_NAME): | $(LOGISTIC_REGRESSION_CHECK)
	-hdfs dfs -rm $(LOGISTIC_REGRESSION_INPUT)
	-hdfs dfs -rm "$(LOGISTIC_REGRESSION_OUTPUT_DIR)/*"
	-hdfs dfs -rmdir $(LOGISTIC_REGRESSION_INPUT_DIR)
	-hdfs dfs -rmdir "$(LOGISTIC_REGRESSION_OUTPUT_DIR)"
	-hdfs dfs -rmdir "$(LOGISTIC_REGRESSION_NAME)"
	-rm $(LOGISTIC_REGRESSION_SENTINELS)
	-rm $(LOGISTIC_REGRESSION_OUTPUT_DIR)/*
	-rm $(LOGISTIC_REGRESSION_OUTPUT_DIR)/.*
	-rmdir $(LOGISTIC_REGRESSION_OUTPUT_DIR)
	-rm $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR)/*
	-rm $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR)/.*
	-rmdir $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR)

submit_$(LOGISTIC_REGRESSION_NAME): $(LOGISTIC_REGRESSION_OUTPUT)


LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR=$(LOGISTIC_REGRESSION_NAME)/staged_output
local_$(LOGISTIC_REGRESSION_NAME):
	echo "LOCAL - Submitting $(LOGISTIC_REGRESSION_EXEC)"
	-rm $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR)/*
	-rm $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR)/.*
	-rmdir $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR)
	spark-submit --name $(LOGISTIC_REGRESSION_NAME) \
	  --master "local[*]"  \
	  $(LOGISTIC_REGRESSION_EXEC) \
	  $(LOGISTIC_REGRESSION_INPUT) \
	  $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR) 2>&1
	cp $(LOGISTIC_REGRESSION_STAGED_OUTPUT_DIR)/* $(LOGISTIC_REGRESSION_OUTPUT_DIR)
	touch $(LOGISTIC_REGRESSION_OUTPUT)

$(LOGISTIC_REGRESSION_HDFS_SENTINEL): | $(LOGISTIC_REGRESSION_CHECK)
	-hdfs dfs -rm $(LOGISTIC_REGRESSION_INPUT)
	-hdfs dfs -rm "$(LOGISTIC_REGRESSION_OUTPUT_DIR)/*"
	-hdfs dfs -rmdir $(LOGISTIC_REGRESSION_INPUT_DIR)
	-mkdir -p $(LOGISTIC_REGRESSION_SENTINEL_DIR)
	hdfs dfs -mkdir -p $(LOGISTIC_REGRESSION_INPUT_DIR)
	touch $(LOGISTIC_REGRESSION_HDFS_SENTINEL)

$(LOGISTIC_REGRESSION_PUSH_SENTINEL): $(LOGISTIC_REGRESSION_INPUT) $(LOGISTIC_REGRESSION_HDFS_SENTINEL) | $(LOGISTIC_REGRESSION_CHECK)
	@echo "Pushing $?"
	-for i in $(filter-out $(LOGISTIC_REGRESSION_SENTINEL_DIR)/%, $?) ; do \
	  echo "Removing $$i"; \
	  hdfs dfs -rm $$i; \
	done
	for i in $(filter-out $(LOGISTIC_REGRESSION_SENTINEL_DIR)/%, $?) ; do \
	...
	touch $(LOGISTIC_REGRESSION_PUSH_SENTINEL)

$(LOGISTIC_REGRESSION_OUTPUT): $(LOGISTIC_REGRESSION_HDFS_SENTINEL) $(LOGISTIC_REGRESSION_PUSH_SENTINEL) $(LOGISTIC_REGRESSION_EXEC) | $(LOGISTIC_REGRESSION_CHECK)
	echo "Submitting $(LOGISTIC_REGRESSION_EXEC)"
	-hdfs dfs -rm "$(LOGISTIC_REGRESSION_OUTPUT_DIR)/*"
	-hdfs dfs -rmdir $(LOGISTIC_REGRESSION_OUTPUT_DIR)
	-mkdir -p $(LOGISTIC_REGRESSION_OUTPUT_DIR)
	spark-submit --name $(LOGISTIC_REGRESSION_NAME) \
	  --master yarn \
	  --deploy-mode cluster \
	  --conf spark.yarn.appMasterEnv.JAVA_HOME=/usr/lib/jvm/java-21-openjdk \
	  ...
	  $(LOGISTIC_REGRESSION_EXEC) \
	  $(LOGISTIC_REGRESSION_INPUT) \
	  $(LOGISTIC_REGRESSION_OUTPUT_DIR) 2>&1
	-rm $(LOGISTIC_REGRESSION_OUTPUT_DIR)/*
	hdfs dfs -get "$(LOGISTIC_REGRESSION_OUTPUT_DIR)/*" $(LOGISTIC_REGRESSION_OUTPUT_DIR)
	touch $(LOGISTIC_REGRESSION_OUTPUT)
